		    ...
		    dbMaster *master)
{
  ...
  for (dbMTerm *mterm : master->getMTerms()) {
	...
	report_->warn(0,"LEF macro %s pin %s missing from liberty cell\n",
		      cell_name,
		      port_name);
    }
  }
  ...
  for (ConcreteLibrary *clib : library_seq_) {
	...
	if (ccell->libertyCell() == nullptr) {
	  ...
	  if (lcell) {
		...
		report_->warn(0,"Liberty cell %s pin %s missing from LEF macro\n",
			      lcell->name(),
			      port_name);
	    }
	    delete port_iter;
...
dbNetwork::deletePin(Pin *pin)
{
  ...
  staToDb(pin, iterm, bterm);
  if (iterm)
    criticalError(0,"not implemented deletePin dbITerm");
  else if (bterm)
...
dbNetwork::mergeInto(Net *,
		     Net *)
{
  criticalError(0,"unimplemented network function mergeInto\n");
}
		   ...
		   dbBTerm *&bterm) const
{
  if (pin) {
    ...
    if (type == dbITermObj) {
      ...
      criticalError(0,"pin is not ITerm or BTerm");
  }
  else {
    iterm = nullptr;
    bterm = nullptr;
		   ...
		   dbIoType &io_type) const
{
  if (dir == PortDirection::input()) {
    ...
    criticalError(0,"unhandled port direction");
}
		   ...
		   dbIoType io_type) const
{
  if (sig_type == dbSigType::POWER)
    ...
    criticalError(0,"unknown master term type");
    return PortDirection::bidirect();
  }
}
